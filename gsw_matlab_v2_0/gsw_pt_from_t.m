function pt = gsw_pt_from_t(SA,t,p,pr)
...
%  pr, from in-situ temperature, t.  This function calls "gsw_entropy_part"
%  which evaluates entropy except for the parts which are a function of 
%  Absolute Salinity alone.
...
%
% OUTPUT:
%  pt   =  potential temperature with reference pressure, pr, on the 
...
elseif (ns == mpr) & (npr == 1)          % pr is a transposed row vector,
    pr = pr';                              % transposed then
    pr = pr(ones(1,ms), :);                % copy down each column.
...
for Number_of_iterations = 1:2
    pt_old = pt;
    dentropy = gsw_entropy_part(SA,pt_old,pr) - true_entropy_part;
    pt = pt_old - dentropy./dentropy_dt ; % this is half way through the modified method
    ptm = 0.5*(pt + pt_old);
    dentropy_dt = -gsw_gibbs(n0,n2,n0,SA,ptm,pr);
