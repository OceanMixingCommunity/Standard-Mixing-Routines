function alpha_wrt_CT = gsw_alpha_wrt_CT(SA,t,p)

% gsw_alpha_wrt_CT                            thermal expansion coefficient 
...
if ~(nargin == 3)
   error('gsw_alpha_wrt_CT:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_alpha_wrt_CT: SA and t must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_alpha_wrt_CT: Inputs array dimensions arguments do not agree')
end %if
...
factor = -cp0./((273.15+pt0).*gsw_gibbs(n0,n2,n0,SA,t,p));

alpha_wrt_CT = factor.*(gsw_gibbs(n0,n1,n1,SA,t,p)./gsw_gibbs(n0,n0,n1,SA,t,p));
