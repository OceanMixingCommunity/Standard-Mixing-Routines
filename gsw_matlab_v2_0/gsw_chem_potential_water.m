function chem_potential_water = gsw_chem_potential_water(SA,t,p)

% gsw_chem_potential_water          chemical potential of water in seawater
%==========================================================================
%
% USAGE:
%  chem_potential_water  = gsw_chem_potential_water(SA,t,p)
%
% DESCRIPTION:
...
%
% OUTPUT:
%  chem_potential_water  =  chemical potential of water in seawater
%                                                                 [ J/kg ]
%
% AUTHOR: 
...
if ~(nargin == 3)
   error('gsw_chem_potential_water:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_chem_potential_water: SA and t must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_chem_potential_water: Inputs array dimensions arguments do not agree')
end %if
...
x = sqrt(x2);
y = t.*0.025;
z = p.*1d-4; %Note that the input pressure (p) is sea pressure in units of dbar.
...
if transposed
    chem_potential_water = chem_potential_water';
end
