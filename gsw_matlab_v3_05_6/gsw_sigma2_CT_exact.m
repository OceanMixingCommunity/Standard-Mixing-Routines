function sigma2_CT_exact = gsw_sigma2_CT_exact(SA,CT)
...
if ~(nargin == 2)
   error('gsw_sigma2_CT_exact:  Requires two inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_sigma2_CT_exact: SA and CT must have same dimensions')
end
...
t = gsw_t_from_CT(SA,CT,pr2000);
sigma2_CT_exact = gsw_rho_t_exact(SA,t,pr2000) - 1000;

if transposed
    sigma2_CT_exact = sigma2_CT_exact.';
end
