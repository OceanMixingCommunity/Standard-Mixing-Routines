function adiabatic_lapse_rate = gsw_adiabatic_lapse_rate_from_t(SA,t,p)
...
if ~(nargin == 3) 
   error('gsw_adiabatic_lapse_rate_from_t:  Requires three inputs')
end
...
if (mt ~= ms | nt ~= ns)
    error('gsw_adiabatic_lapse_rate_from_t: SA and t must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_adiabatic_lapse_rate_from_t: Inputs array dimensions arguments do not agree')
end %if
