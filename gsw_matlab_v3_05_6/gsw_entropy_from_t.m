...
if ~(nargin == 3)
   error('gsw_entropy_from_t:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_entropy_from_t: SA and t must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_entropy_from_t: Inputs array dimensions arguments do not agree')
end %if
