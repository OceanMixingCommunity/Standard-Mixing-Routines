...
if ~(nargin == 5)
    error('gsw_melting_seaice_SA_CT_ratio_poly: Requires five inputs')
end
...
if (mt ~= ms | nt ~= ns)
    error('gsw_melting_seaice_SA_CT_ratio_poly: SA and CT must have same dimensions')
end

if (mssi ~= ms | nssi ~= ns)
    error('gsw_melting_seaice_SA_CT_ratio_poly: SA and SA_seaice must have same dimensions')
end

if (mtsi ~= ms | ntsi ~= ns)
    error('gsw_melting_seaice_SA_CT_ratio_poly: SA and t_seaice must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_melting_seaice_SA_CT_ratio_poly: Inputs array dimensions do not agree; check p')
end 
...
% Note that sea ice is assummed to comprise of Ih ice containing pockets of
% brine (seawater). 
...
% The temperature of the brine trapped in the ice is at the same 
% temperature as the ice (section 4, page 1761 (McDougall et al., 2014)).
