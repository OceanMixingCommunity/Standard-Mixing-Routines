function [specvol, alpha, beta] = gsw_specvol_alpha_beta(SA,CT,p)
...
% 
% USAGE:  
%  [specvol, alpha, beta] = gsw_specvol_alpha_beta(SA,CT,p)
%
% DESCRIPTION:
...
if ~(nargin == 3)
   error('gsw_specvol_alpha_beta:  Requires three inputs')
end %if

if ~(nargout == 3)
   error('gsw_specvol_alpha_beta:  Requires three outputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_specvol_alpha_beta: SA and CT must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_specvol_alpha_beta: Inputs array dimensions arguments do not agree')
end %if
...
specvol = v000 + xs.*(v100 + xs.*(v200 + xs.*(v300 + xs.*(v400 + xs.*(v500 ...
...
beta = -v_SA_part.*0.5.*sfac./(specvol.*xs);

%--------------------------------------------------------------------------
% This function calculates specific volume, alpha and beta using the 
% computationally-efficient expression for specific volume in terms of SA,
% CT and p.  If one wanted to compute specific volume, alpha and beta with
% the full TEOS-10 Gibbs function expression for specific volume, the 
...
%
%   [specvol, alpha, beta] = gsw_specvol_alpha_beta_CT_exact(SA,CT,p)
%
