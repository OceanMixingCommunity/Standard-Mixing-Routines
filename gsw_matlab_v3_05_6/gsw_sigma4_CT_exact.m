function sigma4_CT_exact = gsw_sigma4_CT_exact(SA,CT)
...
% 
% USAGE:  
%  sigma4_CT_exact = gsw_sigma4_CT_exact(SA,CT)
%
% DESCRIPTION:
...
if ~(nargin == 2)
   error('gsw_sigma4_CT_exact:  Requires two inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_sigma4_CT_exact: SA and CT must have same dimensions')
end
...
t = gsw_t_from_CT(SA,CT,pr4000);
sigma4_CT_exact = gsw_rho_t_exact(SA,t,pr4000) - 1000;

if transposed
    sigma4_CT_exact = sigma4_CT_exact.';
end
