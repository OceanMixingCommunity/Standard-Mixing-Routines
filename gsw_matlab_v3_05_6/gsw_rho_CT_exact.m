function rho_CT_exact = gsw_rho_CT_exact(SA,CT,p)
...
if ~(nargin == 3)
   error('gsw_rho_CT_exact:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_rho_CT_exact: SA and CT must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_rho_CT_exact: Inputs array dimensions arguments do not agree')
end %if
