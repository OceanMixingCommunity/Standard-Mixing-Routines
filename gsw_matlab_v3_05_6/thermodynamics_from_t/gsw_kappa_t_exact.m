function kappa_t_exact = gsw_kappa_t_exact(SA,t,p)

% gsw_kappa_t_exact                              isentropic compressibility
%==========================================================================
%
% USAGE:  
%  kappa_t_exact = gsw_kappa_t_exact(SA,t,p)
%
% DESCRIPTION:
...
if ~(nargin == 3)
   error('gsw_kappa_t_exact:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_kappa_t_exact: SA and t must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_kappa_t_exact: Inputs array dimensions arguments do not agree')
end %if
...
kappa_t_exact = (g_tp.*g_tp - g_tt.*gsw_gibbs(0,0,2,SA,t,p))./ ...
