function Helmholtz_energy_t_exact = gsw_Helmholtz_energy_t_exact(SA,t,p)

% gsw_Helmholtz_energy_t_exact                 Helmholtz energy of seawater
%==========================================================================
%
% USAGE:
%  Helmholtz_energy_t_exact = gsw_Helmholtz_energy_t_exact(SA,t,p)
%
% DESCRIPTION:
...
if ~(nargin==3)
   error('gsw_Helmholtz_energy_t_exact:  Requires three inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_Helmholtz_energy_t_exact: SA and t must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_Helmholtz_energy_t_exact: Inputs array dimensions arguments do not agree')
end %if
