function [SA, Sstar, in_ocean] = gsw_SA_Sstar_from_SP(SP,p,long,lat)

% gsw_SA_Sstar_from_SP             Absolute Salinity and Preformed Salinity
...
if ~(nargin == 4)
    error('gsw_SA_Sstar_from_SP:  Requires four inputs')
end %if

if ~(nargout == 2 | nargout == 3)
    error('gsw_SA_Sstar_from_SP:  Requires two or three outputs')
end %if
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_SA_Sstar_from_SP: Inputs array dimensions arguments do not agree')
end %if
...
SA(Iocean) = uPS*SP(Iocean).*(1 + SAAR(Iocean));

Sstar(Iocean) = uPS*SP(Iocean).*(1 - r_1.*SAAR(Iocean));
