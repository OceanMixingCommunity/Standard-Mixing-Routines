function pot_enthalpy = gsw_pot_enthalpy_from_pt(SA,pt)
...
if ~(nargin == 2)
   error('gsw_pot_enthalpy_from_pt:  Requires two inputs')
end %if
...
if (mt ~= ms | nt ~= ns)
    error('gsw_pot_enthalpy_from_pt: SA and pt must have same dimensions')
end
